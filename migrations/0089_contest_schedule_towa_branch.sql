-- 在籍区分: hekibunko → towa_branch（砥部分校）に名称修正
...
CREATE TABLE print_reservations_new (
  ...
  schedule_type TEXT CHECK (
    schedule_type IS NULL OR schedule_type IN ('full_time', 'part_time', 'towa_branch')
  ),
...
SELECT
  ...
  source,
  CASE
    WHEN schedule_type = 'hekibunko' THEN 'towa_branch'
    ELSE schedule_type
  END
