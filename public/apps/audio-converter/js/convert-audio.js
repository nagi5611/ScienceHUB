...
import { isVideoInput } from "./format-utils.js";
...
/**
 * @param {'mp3' | 'm4a' | 'ogg' | 'flac' | 'wav'} format
 ...
 * @param {{ format: 'mp3' | 'm4a' | 'ogg' | 'flac' | 'wav', bitrateKbps: number, extractFromVideo: boolean }} options
 */
async function encodePart(ffmpeg, inputPath, outputName, range, options) {
  ...
  args.push(...codecArgs(options.format, options.bitrateKbps));
 ...
 * 音声を指定形式に変換
 * @param {File} file
 * @param {{ format: 'mp3' | 'm4a' | 'ogg' | 'flac' | 'wav', bitrateKbps: number }} options
 * @param {{ onProgress?: (detail: { phase: string, part?: number, totalParts?: number, ratio?: number, message?: string }) => void }} [callbacks]
 */
export async function convertAudioFile(file, options, callbacks = {}) {
  ...
  const extractFromVideo = isVideoInput(file);

  callbacks.onProgress?.({ phase: "probe", message: "音声を解析中…", ratio: 0.05 });
  const probe = await probeAudio(ffmpeg, inputPath);
  ...
  try {
    for (let i = 0; i < ranges.length; i += 1) {
      ...
      await encodePart(ffmpeg, inputPath, partName, ranges[i], encodeOptions);
    ...
    if (options.format === "mp3") {
      throw new Error(
        "MP3 出力に失敗しました。WAV / M4A / OGG / FLAC をお試しください。" +
