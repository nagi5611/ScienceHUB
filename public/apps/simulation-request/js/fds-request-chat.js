// public/apps/simulation-request/js/fds-request-chat.js
...
/** Mounts FDS request chat UI into a container. */
export function mountFdsRequestChat(container, options) {
  if (!container) return () => {};
  ...
  container.innerHTML = `
    <div class="fds-chat-panel" data-request-id="${escapeHtml(requestId)}">
      <div class="fds-chat-header">
        ...
        <p class="hint fds-chat-hint">シミュレーション規模や MPI 数について相談できます。添付は最大 100MB・7 日間保存。</p>
      </div>
      ...
      ${
        isStaff && canReplaceInput
          ? `<div class="fds-chat-replace-input">
              <p class="fds-chat-replace-label">依頼の入力 .fds を置き換え</p>
              <div class="fds-chat-replace-actions">
                <label class="btn btn-secondary btn-sm">
                  .fds を選択
                  <input type="file" class="fds-chat-replace-file" accept=".fds" hidden />
                </label>
  ...
  /** Replaces request .fds input (staff only). */
  async function replaceInput() {
    const file = replaceFileEl?.files?.[0];
    if (!file) {
      showError('.fds ファイルを選択してください');
      return;
    }
    ...
    if (
      !window.confirm(
        '依頼の入力 .fds を置き換え、一次審査を再実行します。よろしいですか？'
      )
    ) {
      return;
    }
    ...
    try {
      ...
      await apiFormRequest(`admin/fds-requests/${requestId}/replace-input`, formData);
      if (replaceFileEl) replaceFileEl.value = '';
      if (replaceFileNameEl) replaceFileNameEl.textContent = '';
      await loadMessages(false);
      showError('');
      window.dispatchEvent(new CustomEvent('fds-request-input-replaced', { detail: { requestId } }));
    } catch (err) {
...
export function isFdsRequestChatAvailable(status) {
