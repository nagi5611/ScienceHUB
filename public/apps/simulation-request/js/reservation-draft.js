...
export const DRAFT_STORAGE_KEYS = {
  public: '3dsimulator-reservation-draft-v1',
  admin: '3dsimulator-admin-reservation-draft-v1',
};
...
export function extractReservationDraft(form, homeroomField) {
  ...
  return {
    ...
    sim_scale: String(formData.get('sim_scale') ?? ''),
    simulator_id: String(formData.get('simulator_id') ?? ''),
    sim_notes: String(formData.get('sim_notes') ?? ''),
  };
}
...
function applyReservationFormFields(form, data, { purposeOtherGroupId, onApplied } = {}) {
  ...
  setField('sim_notes', data.sim_notes);
  ...
  if (data.sim_scale) {
    const scaleRadio = form.querySelector(`input[name="sim_scale"][value="${data.sim_scale}"]`);
    if (scaleRadio && !scaleRadio.disabled) {
      scaleRadio.checked = true;
    }
  }

  if (data.simulator_id) {
    const simulatorSelect = form.querySelector('[name="simulator_id"]');
    if (simulatorSelect) simulatorSelect.value = data.simulator_id;
  }

  onApplied?.({ purpose: data.purpose, sim_scale: data.sim_scale });
}
