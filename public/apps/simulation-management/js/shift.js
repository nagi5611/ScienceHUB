...
let shiftSimulators = [];
let shiftSimulatorAvailability = [];
...
let pendingSimulatorShiftRemoval = null;
...
function closeShiftRescheduleModal() {
  ...
  pendingSimulatorShiftRemoval = null;
...
function openShiftRescheduleModal(payload) {
  ...
  list.innerHTML = shiftBlockReservations
    .map((r) => {
      const scale = SCALE_LABELS[r.sim_scale] ?? r.sim_scale;
      return `
      <div class="shift-reschedule-item card" style="margin-bottom:0.75rem;padding:0.75rem 1rem">
        ...
        <p>現在の希望実施日: ${escapeHtml(r.desired_date)}</p>
        <div class="form-group" style="margin-bottom:0">
          <label for="shift-reschedule-date-${r.id}">新しい希望実施日</label>
...
async function handleShiftRescheduleSave() {
  ...
  try {
    ...
    } else if (pendingSimulatorShiftRemoval) {
      await apiRequest('admin/shifts/simulator-toggle', {
        method: 'POST',
        body: JSON.stringify({
          simulator_id: pendingSimulatorShiftRemoval.simulatorId,
          date: pendingSimulatorShiftRemoval.date,
        }),
      });
    }
...
export async function renderShiftPanel() {
  ...
  try {
    ...
    shiftSimulators = data.simulators ?? [];
    shiftSimulatorAvailability = data.simulator_availability ?? [];
    ...
    if (shiftEditMode === 'simulator') {
      if (!selectedSimulatorId && shiftSimulators.length) {
        selectedSimulatorId = shiftSimulators[0].id;
      }
      if (selectedSimulatorId && !shiftSimulators.find((p) => p.id === selectedSimulatorId)) {
        selectedSimulatorId = shiftSimulators[0]?.id ?? null;
      }
    }

    renderShiftToolbar();
    renderShiftSimulatorToolbar();
    renderShiftCalendar();
  } catch (err) {
...
function renderShiftToolbar() {
  ...
  if (!shiftMembers.length) {
    toolbar.innerHTML =
      '<p class="hint">メンバー一覧で実行担当者を登録してから、シフトを設定してください。</p>';
    return;
  }
...
/** Builds simulator icon HTML for the shift toolbar. */
function shiftSimulatorIconHtml(simulator) {
...
function renderShiftSimulatorToolbar() {
  const toolbar = document.getElementById('shift-simulator-toolbar');
  if (!toolbar) return;

  if (!shiftSimulators.length) {
    toolbar.innerHTML =
      '<p class="hint">シミュレーター管理で機種を登録してから、稼働日を設定してください。</p>';
    return;
  }

  toolbar.innerHTML = `
    <div class="shift-simulator-toolbar-track">
      ${shiftSimulators
        .map((simulator) => {
          const active =
            shiftEditMode === 'simulator' && simulator.id === selectedSimulatorId
              ? ' shift-simulator-card-active'
              : '';
          return `
            <button type="button" class="shift-simulator-card${active}" data-simulator-id="${simulator.id}">
              ${shiftSimulatorIconHtml(simulator)}
              <span class="shift-simulator-name">${escapeHtml(simulator.name)}</span>
            </button>`;
        })
        .join('')}
    </div>`;

  toolbar.querySelectorAll('[data-simulator-id]').forEach((btn) => {
    btn.addEventListener('click', () => {
      selectedSimulatorId = btn.dataset.simulatorId;
      shiftEditMode = 'simulator';
...
function simulatorAvailabilityByDate() {
  ...
  for (const row of shiftSimulatorAvailability) {
    if (!map[row.date]) map[row.date] = [];
    const simulator = simulatorById[row.simulator_id];
    if (simulator) map[row.date].push(simulator);
  }
...
function renderShiftCalendar() {
  ...
  for (let day = 1; day <= lastDay; day++) {
    ...
    grid.appendChild(createShiftDayCell(day, false, byDate, simulatorsByDate, todayStr, dateStr));
  }
...
function createShiftDayCell(dayNum, otherMonth, byDate, simulatorsByDate, todayStr, dateStr) {
  ...
  if (dateStr && !otherMonth) {
    ...
    if (simulators.length) {
      const simulatorChips = document.createElement('div');
...
function handleShiftDayMouseDown(e, dateStr, cell) {
  ...
  if (shiftEditMode === 'simulator' && !selectedSimulatorId) return;
...
/** Toggles simulator shift; opens reschedule modal when blocked. */
async function toggleSimulatorShiftAvailability(simulatorId, date) {
  try {
    await apiRequest('admin/shifts/simulator-toggle', {
      method: 'POST',
      body: JSON.stringify({ simulator_id: simulatorId, date }),
    });
    return true;
  } catch (err) {
    if (err instanceof ApiError && err.code === 'RESERVATIONS_ON_PRINTER_DATE') {
      pendingSimulatorShiftRemoval = { simulatorId, date };
      pendingShiftRemoval = null;
      openShiftRescheduleModal(err.payload);
      return false;
    }
...
async function handleShiftMouseUp() {
  ...
  if (dates.length === 1 && dates[0] === dragStartDate) {
    try {
      if (shiftEditMode === 'simulator') {
        await toggleSimulatorShiftAvailability(selectedSimulatorId, dates[0]);
      } else {
    ...
    try {
      if (shiftEditMode === 'simulator') {
        await apiRequest('admin/shifts/simulator-availability', {
          method: 'PUT',
          body: JSON.stringify({
      ...
      if (err instanceof ApiError && err.code === 'RESERVATIONS_ON_PRINTER_DATE') {
        pendingSimulatorShiftRemoval = { simulatorId: selectedSimulatorId, date: err.payload.date };
        pendingShiftRemoval = null;
        openShiftRescheduleModal(err.payload);
