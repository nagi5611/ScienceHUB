...
// ../../functions/lib/user-icons.ts
var ICON_MAX_BYTES;
var init_user_icons = __esm({
  "../../functions/lib/user-icons.ts"() {
    "use strict";
    init_r2();
...
// ../../functions/lib/oauth-users.ts
var init_oauth_users = __esm({
  "../../functions/lib/oauth-users.ts"() {
    "use strict";
    init_types();
    init_auth();
...
// ../../functions/lib/third-party/artifacts.ts
function revisionSnapshotPath(revisionNumber) {
  return `revisions/${revisionNumber}/index.html`;
}
function prefixPath(dirName) {
  return `third-party/${dirName}/`;
}
function artifactKey(dirName, name) {
  return `${prefixPath(dirName)}${name}`;
}
async function putArtifact(bucket, dirName, name, body, contentType) {
...
async function getArtifact(bucket, dirName, name) {
...
var LEGACY_REQUIREMENTS, LEGACY_PLAN, ARTIFACT_REQUIREMENTS, ARTIFACT_PLAN, ARTIFACT_TASKS, ARTIFACT_INDEX, ARTIFACT_VERIFY_JSON, ARTIFACT_VERIFY_PNG, DOC_LEGACY_FALLBACK;
var init_artifacts = __esm({
  "../../functions/lib/third-party/artifacts.ts"() {
    "use strict";
    init_auth();
    init_types();
    LEGACY_REQUIREMENTS = "requirements.md";
    LEGACY_PLAN = "implementation-plan.md";
    ARTIFACT_REQUIREMENTS = "docs/requirements.md";
    ARTIFACT_PLAN = "docs/implementation-plan.md";
    ARTIFACT_TASKS = "docs/implementation-tasks.json";
    ARTIFACT_INDEX = "index.html";
    ARTIFACT_VERIFY_JSON = "verify/latest.json";
    ARTIFACT_VERIFY_PNG = "verify/latest.png";
    __name(revisionSnapshotPath, "revisionSnapshotPath");
    DOC_LEGACY_FALLBACK = {
      [ARTIFACT_REQUIREMENTS]: LEGACY_REQUIREMENTS,
      [ARTIFACT_PLAN]: LEGACY_PLAN
    };
    __name(prefixPath, "prefixPath");
    __name(artifactKey, "artifactKey");
    __name(putArtifact, "putArtifact");
    __name(getArtifact, "getArtifact");
  }
});

// ../../functions/lib/third-party/static-analyze.ts
...
function analyzeIndexHtml(html) {
  ...
  const quotes = countChar(combined, '"') % 2 !== 0 || countChar(combined, "'") % 2 !== 0;
...
function formatAnalyzeReport(report) {
  const lines = [
    `\u6982\u8981: ${report.summary}`,
...
var init_static_analyze = __esm({
  "../../functions/lib/third-party/static-analyze.ts"() {
    "use strict";
    __name(extractInlineScripts, "extractInlineScripts");
    __name(countChar, "countChar");
    __name(analyzeIndexHtml, "analyzeIndexHtml");
    __name(formatAnalyzeReport, "formatAnalyzeReport");
  }
});

// ../../functions/lib/third-party/pipeline-client.ts
function isTpPipelineWorkerConfigured(env) {
  return Boolean(env.TP_PIPELINE_WORKER_URL?.trim());
}
async function invokeTpPipelineWorker(env, path, body) {
...
async function triggerVerifyOnWorker(env, projectId, dirName, options) {
...
var init_pipeline_client = __esm({
  "../../functions/lib/third-party/pipeline-client.ts"() {
    "use strict";
    __name(isTpPipelineWorkerConfigured, "isTpPipelineWorkerConfigured");
    __name(invokeTpPipelineWorker, "invokeTpPipelineWorker");
    __name(triggerVerifyOnWorker, "triggerVerifyOnWorker");
  }
});

// ../../functions/lib/third-party/browser-verify.ts
async function storeVerifyReport(bucket, dirName, result) {
  const payload = {
...
function verifyHtmlStatic(html) {
  const report = analyzeIndexHtml(html);
  const errors2 = [];
  const warnings2 = [];
  if (report.brace_balance_warning) {
    errors2.push("\u6CE2\u62EC\u5F27\u306E\u30D0\u30E9\u30F3\u30B9\u304C\u4E0D\u6B63\u306E\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059");
  }
  if (report.quote_balance_warning) {
    errors2.push("\u5F15\u7528\u7B26\u306E\u30D0\u30E9\u30F3\u30B9\u304C\u4E0D\u6B63\u306E\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059");
  }
  if (!/<html\b/i.test(html) || !/<\/html>/i.test(html)) {
    errors2.push("HTML \u30C9\u30AD\u30E5\u30E1\u30F3\u30C8\u69CB\u9020\u304C\u4E0D\u5B8C\u5168\u3067\u3059");
  }
  warnings2.push(report.summary);
  if (formatAnalyzeReport(report).includes("clear")) {
    warnings2.push("\u30AF\u30EA\u30A2\u95A2\u9023\u306E\u30B3\u30FC\u30C9\u3092\u691C\u51FA\u3057\u307E\u3057\u305F\uFF08\u52D5\u4F5C\u78BA\u8A8D\u63A8\u5968\uFF09");
  }
  return {
...
async function verifyProjectHtml(env, bucket, projectId, dirName, options) {
  ...
  if (isTpPipelineWorkerConfigured(env)) {
    ...
    if (workerResult.ok && workerResult.body && typeof workerResult.body === "object") {
      const b = workerResult.body;
      const result = {
        passed: Boolean(b.passed),
        errors: Array.isArray(b.errors) ? b.errors : [],
        warnings: Array.isArray(b.warnings) ? b.warnings : [],
        title: typeof b.title === "string" ? b.title : void 0,
        bodyLength: typeof b.bodyLength === "number" ? b.bodyLength : void 0,
...
var VERIFY_JSON_SCHEMA_VERSION;
var init_browser_verify = __esm({
  "../../functions/lib/third-party/browser-verify.ts"() {
    "use strict";
    init_artifacts();
    init_static_analyze();
    ...
    __name(storeVerifyReport, "storeVerifyReport");
    __name(verifyHtmlStatic, "verifyHtmlStatic");
    __name(verifyProjectHtml, "verifyProjectHtml");
  }
});

// ../../functions/lib/gemini/generate.ts
...
function serviceTierToApi(tier) {
  switch (tier) {
    case "FLEX":
      return "flex";
    case "PRIORITY":
...
function logGeminiUsage(label, model, usage) {
  if (!usage) return;
  console.log(
    JSON.stringify({
      type: "gemini_usage",
...
function parseJsonFromModelText(text) {
  ...
  try {
    ...
    if (!jsonMatch) {
      const hint = firstError instanceof Error && firstError.message.includes("Unterminated") ? "AI \u306E JSON \u304C\u9014\u4E2D\u3067\u5207\u308C\u3066\u3044\u307E\u3059\uFF08\u51FA\u529B\u304C\u9577\u3059\u304E\u308B\u53EF\u80FD\u6027\u304C\u3042\u308A\u307E\u3059\uFF09" : "AI \u306E JSON \u3092\u89E3\u91C8\u3067\u304D\u307E\u305B\u3093\u3067\u3057\u305F";
...
async function geminiGenerateText(env, options) {
...
async function geminiGenerateJson(env, options) {
...
var init_generate = __esm({
  "../../functions/lib/gemini/generate.ts"() {
    ...
    __name(serviceTierToApi, "serviceTierToApi");
    __name(logGeminiUsage, "logGeminiUsage");
    __name(getGeminiApiKey, "getGeminiApiKey");
    __name(parseJsonFromModelText, "parseJsonFromModelText");
    __name(geminiGenerateText, "geminiGenerateText");
    __name(geminiGenerateJson, "geminiGenerateJson");
  }
});

// ../../functions/lib/gemini/context-cache.ts
async function createTpImplementDocsCache(env, model, systemInstruction, requirements, plan) {
...
var MIN_DOCS_CHARS_FOR_EXPLICIT_CACHE, DEFAULT_CACHE_TTL_SECONDS;
var init_context_cache = __esm({
  "../../functions/lib/gemini/context-cache.ts"() {
    "use strict";
    init_generate();
    MIN_DOCS_CHARS_FOR_EXPLICIT_CACHE = 8e3;
    DEFAULT_CACHE_TTL_SECONDS = 3600;
    __name(createTpImplementDocsCache, "createTpImplementDocsCache");
  }
});

// ../../functions/lib/third-party/prompts.ts
var FLASH_IMPLEMENT_TASK_EDIT_SYSTEM;
var init_prompts = __esm({
  "../../functions/lib/third-party/prompts.ts"() {
    "use strict";
    FLASH_IMPLEMENT_TASK_EDIT_SYSTEM = `ScienceHUB \u30B5\u30FC\u30C9\u30D1\u30FC\u30C6\u30A3\u306E\u6BB5\u968E\u5B9F\u88C5\u30A8\u30C7\u30A3\u30BF\u3002
...
// ../../functions/lib/third-party/schemas.ts
var WORKSPACE_EDIT_OP_SCHEMA, MAINTAIN_EDIT_PLAN_SCHEMA, IMPLEMENT_EDIT_PLAN_SCHEMA, IMPLEMENTATION_TASKS_PLAN_SCHEMA;
var init_schemas = __esm({
  "../../functions/lib/third-party/schemas.ts"() {
    "use strict";
    WORKSPACE_EDIT_OP_SCHEMA = {
      type: "OBJECT",
      properties: {
        op: {
          type: "STRING",
          description: "replace_lines | insert_after | insert_before | delete_lines"
        },
        start_line: {
          type: "INTEGER",
          nullable: true,
        ...
        content: {
          type: "STRING",
          nullable: true,
          description: "\u633F\u5165\u30FB\u7F6E\u63DB\u5F8C\u306E\u30C6\u30AD\u30B9\u30C8\u3002\u6539\u884C\u53EF\u3002delete_lines \u3067\u306F\u4E0D\u8981"
        }
      },
      required: ["op"]
    };
    ...
    IMPLEMENT_EDIT_PLAN_SCHEMA = {
      ...
      properties: {
        ...
        edits: {
          type: "ARRAY",
          description: "target_path \u306E\u30D5\u30A1\u30A4\u30EB\u3078\u306E\u884C\u5358\u4F4D\u7DE8\u96C6\u3002L001 \u306E\u884C\u756A\u53F7\u3068\u4E00\u81F4\u3002content \u306F\u8A72\u5F53\u30D6\u30ED\u30C3\u30AF\u5185\u306E\u30C6\u30AD\u30B9\u30C8\u306E\u307F",
          items: WORKSPACE_EDIT_OP_SCHEMA
        }
      },
...
// ../../functions/lib/third-party/workspace-edits.ts
...
function validateEditOp(op, lineCount, index) {
  const contentLen = /* @__PURE__ */ __name((c) => (c ?? "").length, "contentLen");
  switch (op.op) {
    case "replace_lines": {
...
function applyWorkspaceEdits(text, edits) {
  if (!edits.length) {
...
function normalizeWorkspaceEdits(raw) {
  ...
  for (const item of raw) {
    ...
    const op = rec.op;
    if (op === "replace_lines") {
      if (typeof rec.start_line !== "number" || typeof rec.end_line !== "number" || typeof rec.content !== "string") {
        return null;
      }
      out.push({
        op: "replace_lines",
...
var MAX_EDIT_OPS, MAX_EDIT_CONTENT_CHARS;
var init_workspace_edits = __esm({
  "../../functions/lib/third-party/workspace-edits.ts"() {
    "use strict";
    MAX_EDIT_OPS = 20;
    MAX_EDIT_CONTENT_CHARS = 8192;
    __name(formatNumberedLines, "formatNumberedLines");
    __name(validateEditOp, "validateEditOp");
    __name(applyWorkspaceEdits, "applyWorkspaceEdits");
    __name(editSortKey, "editSortKey");
    __name(summarizeEdits, "summarizeEdits");
    __name(normalizeWorkspaceEdits, "normalizeWorkspaceEdits");
  }
});

// ../../functions/lib/third-party/stub-chat.ts
var EMPTY_PLACEHOLDER_HTML;
var init_stub_chat = __esm({
  "../../functions/lib/third-party/stub-chat.ts"() {
    "use strict";
    EMPTY_PLACEHOLDER_HTML = `<!DOCTYPE html>
<html lang="ja">
...
// ../../functions/lib/third-party/implement-edit-scope.ts
function canParallelizeTargets(a, b) {
  if (a === b) return false;
...
function maxParallelBatchSize() {
  return MAX_PARALLEL_BATCH;
}
function findStyleBlock(lines) {
...
function findLastScriptBlock(lines) {
  let last = null;
  for (let i = 0; i < lines.length; i++) {
...
function blockInnerLines1Based(block) {
...
function formatLineRange(label, start, end) {
  return `${label}: L${start}\u2013L${end}`;
}
function describeImplementEditScopes(html, taskTarget) {
...
function lineInRange(line, start, end) {
  return line >= start && line <= end;
}
function contentForbiddenForTarget(content, taskTarget) {
  const c = content;
...
function validateImplementEdits(html, edits, taskTarget) {
  ...
  for (let i = 0; i < edits.length; i++) {
    const op = edits[i];
    const content = op.op === "replace_lines" || op.op === "insert_after" || op.op === "insert_before" ? op.content : "";
...
var IMPLEMENT_EDIT_TARGET_PATH, PARALLEL_SAFE_TARGETS, MAX_PARALLEL_BATCH;
var init_implement_edit_scope = __esm({
  "../../functions/lib/third-party/implement-edit-scope.ts"() {
    "use strict";
    IMPLEMENT_EDIT_TARGET_PATH = "index.html";
    PARALLEL_SAFE_TARGETS = /* @__PURE__ */ new Set(["markup", "styles"]);
    MAX_PARALLEL_BATCH = 3;
    __name(canParallelizeTargets, "canParallelizeTargets");
    __name(maxParallelBatchSize, "maxParallelBatchSize");
    __name(findStyleBlock, "findStyleBlock");
    __name(findLastScriptBlock, "findLastScriptBlock");
    __name(findMainBlock, "findMainBlock");
    __name(blockInnerLines1Based, "blockInnerLines1Based");
    __name(formatLineRange, "formatLineRange");
    __name(describeImplementEditScopes, "describeImplementEditScopes");
    __name(lineInRange, "lineInRange");
    __name(contentForbiddenForTarget, "contentForbiddenForTarget");
    __name(validateImplementEdits, "validateImplementEdits");
  }
});

// ../../functions/lib/third-party/tp-flash.ts
function resolveTpFlashModel(env) {
  return env.GEMINI_TP_FLASH_MODEL?.trim() || DEFAULT_TP_FLASH_MODEL;
}
function tpGeminiProfileOptions(profile) {
  switch (profile) {
    case "lite_turn":
    case "lite_docs":
      return {
...
var DEFAULT_TP_FLASH_MODEL, TP_LITE_THINKING, TP_FLASH_THINKING_EDIT, TP_FLASH_THINKING_EDIT_RETRY, TP_FLASH_THINKING_SNIPPET, TP_FLASH_THINKING_PATCH, TP_FLASH_THINKING_AGENT_STEP, TP_FLASH_THINKING_STRUCTURED, TP_FLASH_THINKING_ASK, TP_INTERACTIVE_SERVICE_TIER;
var init_tp_flash = __esm({
  "../../functions/lib/third-party/tp-flash.ts"() {
    "use strict";
    DEFAULT_TP_FLASH_MODEL = "gemini-3.5-flash";
    __name(resolveTpFlashModel, "resolveTpFlashModel");
    TP_LITE_THINKING = "MINIMAL";
    TP_FLASH_THINKING_EDIT = "MEDIUM";
    TP_FLASH_THINKING_EDIT_RETRY = "HIGH";
    TP_FLASH_THINKING_SNIPPET = "MEDIUM";
    TP_FLASH_THINKING_PATCH = "HIGH";
    TP_FLASH_THINKING_AGENT_STEP = "LOW";
    TP_FLASH_THINKING_STRUCTURED = "LOW";
    TP_FLASH_THINKING_ASK = "LOW";
    TP_INTERACTIVE_SERVICE_TIER = "STANDARD";
    __name(tpGeminiProfileOptions, "tpGeminiProfileOptions");
  }
});

// ../../functions/lib/third-party/implement-tasks.ts
function escapeHtmlText(value) {
  return value.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");
}
function buildProjectSkeleton(title) {
...
function isBareOrDefaultSkeleton(html) {
  const t = html.trim();
  if (!t) return true;
  if (t === EMPTY_SKELETON.trim()) return true;
  if (isScienceHubPlaceholderHtml(t)) return true;
  return t.length < 500 && t.includes("<body") && !t.includes('id="app"') && !t.includes("<main");
}
function isScienceHubPlaceholderHtml(html) {
  const a = html.trim();
  if (a === EMPTY_PLACEHOLDER_HTML.trim()) return true;
  return a.includes("\u5DE6\u306E\u30C1\u30E3\u30C3\u30C8\u3067") && a.includes("\u30E9\u30F3\u30C7\u30A3\u30F3\u30B0\u30DA\u30FC\u30B8") && !/<style\b/i.test(a) && !/<script\b/i.test(a);
}
function normalizeImplementBaseHtml(html, title) {
  const t = html.trim();
...
async function saveImplementationTasks(bucket, dirName, file) {
  await putArtifact(
    bucket,
    dirName,
    ARTIFACT_TASKS,
...
async function planImplementationTasks(env, requirements, plan, title) {
...
\u4F8B: skeleton \u2192 \u30EC\u30A4\u30A2\u30A6\u30C8 markup \u2192 UI \u2192 styles \u2192 script \u2192 polish`;
  const result = await geminiGenerateJson(env, {
...
id \u306F\u82F1\u5C0F\u6587\u5B57\u3068\u30CF\u30A4\u30D5\u30F3\uFF08\u4F8B: task-skeleton\uFF09\u3002depends_on \u306F\u5148\u884C\u30BF\u30B9\u30AF id \u306E\u914D\u5217\u3002`,
    ...
    responseSchema: IMPLEMENTATION_TASKS_PLAN_SCHEMA
  });
  const tasks = (result.tasks ?? []).slice(0, MAX_TASKS).map((t) => ({
    id: t.id,
    title: t.title,
...
function normalizeTaskTarget(value) {
  ...
  return allowed.includes(value) ? value : "markup";
}
async function prepareImplementGeminiContext(env, requirements, plan) {
  ...
  return { docsCacheName: name ?? void 0 };
}
async function generateEditPlan(env, systemInstruction, prompt, options) {
  const attempt = options?.attempt ?? 0;
  const profile = attempt > 0 ? "flash_edit_plan_retry" : "flash_edit_plan";
  const schema = options?.planKind === "implement" ? IMPLEMENT_EDIT_PLAN_SCHEMA : MAINTAIN_EDIT_PLAN_SCHEMA;
  return await geminiGenerateJson(env, {
    model: resolveTpFlashModel(env),
    systemInstruction: options?.docsCacheName ? void 0 : systemInstruction,
    prompt,
...
function isTokenLimitError(message) {
  return message.includes("\u9577\u3059\u304E\u3066") || message.includes("MAX_TOKENS") || message.includes("\u9014\u4E2D\u3067\u5207\u308C");
}
function buildImplementTaskEditPrompt(currentHtml, task, title, requirements, plan, docsCache, retryNote) {
...
\u5B8C\u4E86\u6761\u4EF6: ${task.acceptance_hint}`;
  const docsBlock = docsCache ? `${taskBlock}

\u4E0A\u8A18\u306E\u5168\u6587\u3068\u30AD\u30E3\u30C3\u30B7\u30E5\u6E08\u307F\u8981\u4EF6\u30FB\u8A08\u753B\u306B\u57FA\u3065\u304D\u3001\u3053\u306E\u30BF\u30B9\u30AF\u3060\u3051\u3092\u6E80\u305F\u3059 edits \u3092\u8FD4\u3057\u3066\u304F\u3060\u3055\u3044\u3002
target_path \u306F\u5FC5\u305A "${IMPLEMENT_EDIT_TARGET_PATH}"\u3002` : `--- \u8981\u4EF6\uFF08\u629C\u7C8B\uFF09 ---
${requirements.slice(0, 4e3)}
...
async function applyImplementationTask(env, currentHtml, task, requirements, plan, title, callbacks) {
  if (isSkeletonLikeTask(task) && (isBareOrDefaultSkeleton(currentHtml) || isScienceHubPlaceholderHtml(currentHtml))) {
  ...
  for (let attempt = 0; attempt < MAX_TASK_EDIT_RETRIES; attempt++) {
    callbacks?.onActivity?.(
      attempt === 0 ? `${task.title}\u2026` : `${task.title}\uFF08\u518D\u8A66\u884C ${attempt + 1}\uFF09\u2026`
    );
    const retryNote = attempt === 0 ? void 0 : `\u524D\u56DE\u306E\u30A8\u30E9\u30FC: ${lastError}
...
target_path \u306F "${IMPLEMENT_EDIT_TARGET_PATH}"\u3002\u884C\u756A\u53F7\u306F\u4E0A\u8A18\u5168\u6587\u306E L001 \u3068\u4E00\u81F4\u3055\u305B\u308B\u3002`;
    ...
    try {
      ...
      lastError = isTokenLimitError(msg) ? `${msg}\uFF08\u5168\u6587\u3092\u898B\u305F\u4E0A\u3067 edits \u306E\u307F\u8FD4\u3057\u3001\u30B9\u30CB\u30DA\u30C3\u30C8\u5358\u4F53\u51FA\u529B\u306F\u3057\u306A\u3044\uFF09` : msg;
      continue;
    }
    ...
    return {
      ...
      assistantMessage: planResult.assistant_message?.trim() || `${task.title} \u3092\u53CD\u6620\u3057\u307E\u3057\u305F\u3002`
    };
  }
  throw new Error(lastError);
}
async function planImplementationTaskEdits(env, currentHtml, task, requirements, plan, title, gemini) {
  currentHtml = normalizeImplementBaseHtml(currentHtml, title);
  const prompt = buildImplementTaskEditPrompt(
    currentHtml,
    task,
...
function applyPlannedTaskEdits(currentHtml, task, planResult, _title) {
  const edits = normalizeWorkspaceEdits(planResult.edits);
  if (!edits?.length) {
...
var EMPTY_SKELETON, MAX_TASKS, MAX_TASK_EDIT_RETRIES, MAX_EDIT_PLAN_TOKENS;
var init_implement_tasks = __esm({
  "../../functions/lib/third-party/implement-tasks.ts"() {
    "use strict";
    init_generate();
    init_context_cache();
    ...
    init_prompts();
    init_schemas();
    init_workspace_edits();
    init_stub_chat();
    init_implement_edit_scope();
    init_tp_flash();
    EMPTY_SKELETON = `<!DOCTYPE html>
<html lang="ja">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
...
<body>
    ...
    MAX_TASKS = 5;
    MAX_TASK_EDIT_RETRIES = 3;
    MAX_EDIT_PLAN_TOKENS = 16384;
    __name(escapeHtmlText, "escapeHtmlText");
    __name(buildProjectSkeleton, "buildProjectSkeleton");
    __name(isBareOrDefaultSkeleton, "isBareOrDefaultSkeleton");
    __name(isScienceHubPlaceholderHtml, "isScienceHubPlaceholderHtml");
    __name(normalizeImplementBaseHtml, "normalizeImplementBaseHtml");
    __name(stripScienceHubPlaceholderParagraph, "stripScienceHubPlaceholderParagraph");
    __name(isSkeletonLikeTask, "isSkeletonLikeTask");
    __name(isCompleteIndexHtml, "isCompleteIndexHtml");
    __name(loadImplementationTasks, "loadImplementationTasks");
    __name(saveImplementationTasks, "saveImplementationTasks");
    __name(planImplementationTasks, "planImplementationTasks");
    __name(normalizeTaskTarget, "normalizeTaskTarget");
    __name(prepareImplementGeminiContext, "prepareImplementGeminiContext");
    __name(generateEditPlan, "generateEditPlan");
    __name(isTokenLimitError, "isTokenLimitError");
    __name(buildImplementTaskEditPrompt, "buildImplementTaskEditPrompt");
    __name(applyImplementationTask, "applyImplementationTask");
    __name(planImplementationTaskEdits, "planImplementationTaskEdits");
    __name(applyPlannedTaskEdits, "applyPlannedTaskEdits");
  }
});

// ../../functions/lib/third-party/implement-parallel.ts
...
function batchCanAdd(batch, candidate) {
  for (const t of batch) {
    if (!canParallelizeTargets(t.target, candidate.target)) return false;
  }
  return true;
}
function hasPendingTasks(tasksFile) {
  return tasksFile.tasks.some((t) => t.status === "pending");
}
function nextParallelBatch(tasksFile) {
  const ready = tasksFile.tasks.filter(
    (t) => isTaskReady(t, tasksFile.tasks)
  );
...
var init_implement_parallel = __esm({
  "../../functions/lib/third-party/implement-parallel.ts"() {
    "use strict";
    init_implement_edit_scope();
    __name(isTaskReady, "isTaskReady");
    __name(batchCanAdd, "batchCanAdd");
    __name(hasPendingTasks, "hasPendingTasks");
    __name(nextParallelBatch, "nextParallelBatch");
  }
});

// ../../functions/lib/third-party/project-html.ts
async function writeProjectIndexHtml(bucket, project, html) {
  await putArtifact(
    bucket,
    project.dir_name,
    ARTIFACT_INDEX,
...
var init_project_html = __esm({
  "../../functions/lib/third-party/project-html.ts"() {
    "use strict";
    init_artifacts();
    __name(writeProjectIndexHtml, "writeProjectIndexHtml");
  }
});

// ../../functions/lib/third-party/revisions.ts
...
async function recordHtmlRevision(db, bucket, project, html, summary, jobId) {
...
async function snapshotCurrentHtml(db, bucket, project, summary, jobId) {
...
var init_revisions = __esm({
  "../../functions/lib/third-party/revisions.ts"() {
    ...
    init_artifacts();
    init_project_html();
    __name(nextRevisionNumber, "nextRevisionNumber");
    __name(recordHtmlRevision, "recordHtmlRevision");
    __name(snapshotCurrentHtml, "snapshotCurrentHtml");
  }
});

// ../../functions/lib/third-party/jobs.ts
async function getTpJob(db, jobId) {
...
async function updateJobProgress(db, jobId, progress) {
  await db.prepare("UPDATE tp_jobs SET progress_json = ? WHERE id = ?").bind(JSON.stringify(progress), jobId).run();
}
async function markJobRunning(db, jobId) {
  await db.prepare(
    "UPDATE tp_jobs SET status = 'running', started_at = ? WHERE id = ?"
  ).bind(now(), jobId).run();
}
async function markJobSucceeded(db, jobId, projectId, result) {
  const finished = now();
  await db.prepare(
    `UPDATE tp_jobs SET status = 'succeeded', result_json = ?, finished_at = ?
       WHERE id = ?`
  ).bind(result ? JSON.stringify(result) : null, finished, jobId).run();
  await db.prepare(
    "UPDATE tp_projects SET active_job_id = NULL WHERE id = ? AND active_job_id = ?"
  ).bind(projectId, jobId).run();
}
async function markJobFailed(db, jobId, projectId, errorMessage) {
  const finished = now();
  await db.prepare(
    `UPDATE tp_jobs SET status = 'failed', error_message = ?, finished_at = ?
       WHERE id = ?`
  ).bind(errorMessage.slice(0, 500), finished, jobId).run();
  await db.prepare(
    "UPDATE tp_projects SET active_job_id = NULL WHERE id = ? AND active_job_id = ?"
  ).bind(projectId, jobId).run();
}
...
var init_jobs = __esm({
  "../../functions/lib/third-party/jobs.ts"() {
    "use strict";
    init_types();
    TP_JOB_STALE_MS = 15 * 60 * 1e3;
    __name(getTpJob, "getTpJob");
    __name(updateJobProgress, "updateJobProgress");
    __name(markJobRunning, "markJobRunning");
    __name(markJobSucceeded, "markJobSucceeded");
    __name(markJobFailed, "markJobFailed");
  }
});

// ../../functions/lib/third-party/implement-runner.ts
async function insertAssistantMessage(db, projectId, content) {
  await db.prepare(
    `INSERT INTO tp_chat_messages (id, project_id, role, content, created_at)
       VALUES (?, ?, 'assistant', ?, ?)`
  ).bind(createId("tpmsg"), projectId, content, now()).run();
}
async function patchProjectPhase(db, projectId, fields) {
  ...
  await db.prepare(`UPDATE tp_projects SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
}
async function runImplementJob(env, db, bucket, project, jobId, callbacks) {
  ...
  const requirements = await getArtifact(bucket, project.dir_name, ARTIFACT_REQUIREMENTS) ?? "";
  ...
  const emitTasks = /* @__PURE__ */ __name(() => {
    callbacks?.onTasks?.({
  ...
  emitTasks();
  let html = await getArtifact(bucket, project.dir_name, ARTIFACT_INDEX) ?? "";
  ...
  while (hasPendingTasks(tasksFile)) {
    ...
    if (batch.length > 1) {
      const plans = await Promise.all(
        batch.map((task) => {
          if (isSkeletonLikeTask(task) && (isBareOrDefaultSkeleton(html) || isScienceHubPlaceholderHtml(html))) {
          ...
          return planImplementationTaskEdits(
            ...
            project.title,
            implementGemini
          ).then((planResult) => ({ skeleton: false, planResult, task }));
        })
      );
      ...
      for (const task of batch) {
        const result = await applyImplementationTask(
          ...
          project.title,
          {
            onActivity: /* @__PURE__ */ __name((label) => callbacks?.onActivity?.(label, "flash_implement_tasks"), "onActivity"),
  ...
  const assistantMessage = summaries.length > 0 ? summaries[summaries.length - 1] : "index.html \u306E\u6BB5\u968E\u5B9F\u88C5\u304C\u5B8C\u4E86\u3057\u307E\u3057\u305F\u3002";
  const verifyNote = verifyResult.passed ? "" : `\uFF08\u30D6\u30E9\u30A6\u30B6\u691C\u8A3C\u3067\u554F\u984C: ${verifyResult.errors.slice(0, 2).join("; ")}\uFF09`;
...
async function failImplementJob(db, jobId, projectId, error) {
  await markJobFailed(db, jobId, projectId, error);
  await recoverPhaseAfterImplementFailure(db, projectId, error);
}
async function recoverPhaseAfterImplementFailure(db, projectId, errorMessage) {
...
var MAX_IMPLEMENT_ATTEMPTS;
var init_implement_runner = __esm({
  "../../functions/lib/third-party/implement-runner.ts"() {
    "use strict";
    init_types();
    init_artifacts();
    init_implement_tasks();
    init_implement_parallel();
    init_project_html();
    init_revisions();
    init_jobs();
    init_browser_verify();
    MAX_IMPLEMENT_ATTEMPTS = 3;
    __name(insertAssistantMessage, "insertAssistantMessage");
    __name(patchProjectPhase, "patchProjectPhase");
    __name(runImplementJob, "runImplementJob");
    __name(failImplementJob, "failImplementJob");
    __name(recoverPhaseAfterImplementFailure, "recoverPhaseAfterImplementFailure");
  }
});
...
var FrameManagerEmittedEvents = {
  ...
  ExecutionContextCreated: /* @__PURE__ */ Symbol("FrameManager.ExecutionContextCreated"),
  ExecutionContextDestroyed: /* @__PURE__ */ Symbol("FrameManager.ExecutionContextDestroyed")
};
...
var IsolatedWorld = class _IsolatedWorld {
  ...
  constructor(frame) {
    ...
    _IsolatedWorld_onBindingCalled.set(this, async (event) => {
      ...
      if (type !== "internal" || !__classPrivateFieldGet11(this, _IsolatedWorld_ctxBindings, "f").has(__classPrivateFieldGet11(_IsolatedWorld, _a, "f", _IsolatedWorld_bindingIdentifier).call(_IsolatedWorld, name, context._contextId))) {
        return;
      }
  ...
  async _addBindingToContext(context, name) {
    if (__classPrivateFieldGet11(this, _IsolatedWorld_ctxBindings, "f").has(__classPrivateFieldGet11(_IsolatedWorld, _a, "f", _IsolatedWorld_bindingIdentifier).call(_IsolatedWorld, name, context._contextId))) {
      return;
    }
    ...
    const bind = /* @__PURE__ */ __name(async (name2) => {
      ...
      __classPrivateFieldGet11(this, _IsolatedWorld_ctxBindings, "f").add(__classPrivateFieldGet11(_IsolatedWorld, _a, "f", _IsolatedWorld_bindingIdentifier).call(_IsolatedWorld, name2, context._contextId));
    }, "bind");
...
_Page_closed = /* @__PURE__ */ new WeakMap(), _Page_client = /* @__PURE__ */ new WeakMap(), _Page_target = /* @__PURE__ */ new WeakMap(), _Page_keyboard = /* @__PURE__ */ new WeakMap(), _Page_mouse = /* @__PURE__ */ new WeakMap(), _Page_timeoutSettings = /* @__PURE__ */ new WeakMap(), _Page_touchscreen = /* @__PURE__ */ new WeakMap(), _Page_accessibility = /* @__PURE__ */ new WeakMap(), _Page_frameManager = /* @__PURE__ */ new WeakMap(), _Page_emulationManager = /* @__PURE__ */ new WeakMap(), _Page_pageBindings = /* @__PURE__ */ new WeakMap(), _Page_coverage = /* @__PURE__ */ new WeakMap(), _Page_javascriptEnabled = /* @__PURE__ */ new WeakMap(), _Page_viewport = /* @__PURE__ */ new WeakMap(), _Page_screenshotTaskQueue = /* @__PURE__ */ new WeakMap(), _Page_workers = /* @__PURE__ */ new WeakMap(), _Page_fileChooserPromises = /* @__PURE__ */ new WeakMap(), _Page_disconnectPromise = /* @__PURE__ */ new WeakMap(), _Page_userDragInterceptionEnabled = /* @__PURE__ */ new WeakMap(), _Page_handlerMap = /* @__PURE__ */ new WeakMap(), _Page_onDetachedFromTarget = /* @__PURE__ */ new WeakMap(), _Page_onAttachedToTarget = /* @__PURE__ */ new WeakMap(), _Page_instances = /* @__PURE__ */ new WeakSet(), _Page_initialize = /* @__PURE__ */ __name(async function _Page_initialize2() {
  ...
  const { level: level2, text, args, source, url, lineNumber } = event.entry;
  if (args) {
    args.map((arg) => {
...
async function loadProjectForJob(db, projectId) {
  const row = await db.prepare(
...
__name(loadProjectForJob, "loadProjectForJob");
async function runBrowserVerify(env, dirName, html) {
  const staticFallback = verifyHtmlStatic(html);
  try {
    const browser = await puppeteer_core_default.launch(env.BROWSER);
    const page = await browser.newPage();
    const errors2 = [...staticFallback.errors];
    const warnings2 = [...staticFallback.warnings];
    page.on("pageerror", (err) => {
      errors2.push(`pageerror: ${err.message}`);
    });
    page.on("console", (msg) => {
      if (msg.type() === "error") {
        errors2.push(`console: ${msg.text()}`);
      }
    });
    ...
    try {
      ...
      warnings2.push("\u30B9\u30AF\u30EA\u30FC\u30F3\u30B7\u30E7\u30C3\u30C8\u306E\u4FDD\u5B58\u306B\u5931\u6557\u3057\u307E\u3057\u305F");
    }
    await browser.close();
    return {
      passed: errors2.length === 0,
...
__name(runBrowserVerify, "runBrowserVerify");
...
async function runImplementInBackground(env, jobId, projectId) {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "\u5B9F\u88C5\u30B8\u30E7\u30D6\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
    await failImplementJob(env.DB, jobId, projectId, msg);
  }
}
__name(runImplementInBackground, "runImplementInBackground");
async function handleImplement(request, env, ctx) {
  const body = await request.json();
...
__name(handleImplement, "handleImplement");
var index_default = {
