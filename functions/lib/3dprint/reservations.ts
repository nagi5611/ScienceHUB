...
export interface Reservation {
  ...
  status: 'applied' | 'accepted' | 'printing' | 'delivered' | 'failed' | 'cancelled';
  status_comment: string | null;
  print_staff: string | null;
...
export async function hasReservationsBeforeDate(
  ...
  const row = await db
    .prepare(
      `SELECT 1 AS ok FROM print_reservations
       WHERE desired_date < ? AND status != 'cancelled'
       LIMIT 1`
    )
    .bind(beforeDate)
...
export async function updateReservationAdmin(
  ...
  if (fields.print_staff_member_id !== undefined) {
    sets.push('print_staff_member_id = ?');
    values.push(fields.print_staff_member_id);
  }
  ...
  values.push(id);
  await db.prepare(`UPDATE print_reservations SET ${sets.join(', ')} WHERE id = ?`).bind(...values)    .run();
}
...
export async function setGoogleEventId(
  ...
  await db
    .prepare('UPDATE print_reservations SET google_event_id = ? WHERE id = ?')
...
export async function acceptReservation(
  ...
  const result = await db
    .prepare(
      `UPDATE print_reservations SET status = 'accepted', print_staff_member_id = ?
...
export async function getAllMembers(db: D1Database): Promise<Member[]> {
  const result = await db
    .prepare(`SELECT * FROM print_members ORDER BY homeroom, student_number`)
    .all<Member>();
  return result.results ?? [];
}
...
export async function getMemberById(db: D1Database, id: string): Promise<Member | null> {
  return db.prepare('SELECT * FROM print_members WHERE id = ?').bind(id).first<Member>();
}
...
export async function createMember(db: D1Database, data: Member): Promise<void> {
  await db
    .prepare(
      `INSERT INTO print_members (id, homeroom, student_number, name, color_index, discord_user_id, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      data.id,
...
export async function updateMemberDiscordUserId(
  ...
  await db
    .prepare('UPDATE print_members SET discord_user_id = ? WHERE id = ?')
...
export async function updateMemberColor(db: D1Database, id: string, colorIndex: number): Promise<void> {
  await db.prepare('UPDATE print_members SET color_index = ? WHERE id = ?').bind(colorIndex, id).run();
}

/** Deletes a member by ID. */
export async function deleteMember(db: D1Database, id: string): Promise<boolean> {
  await db
    .prepare('UPDATE print_reservations SET print_staff_member_id = NULL WHERE print_staff_member_id = ?')
    .bind(id)
    .run();
  await db.prepare('DELETE FROM print_member_availability WHERE member_id = ?').bind(id).run();
  const result = await db.prepare('DELETE FROM print_members WHERE id = ?').bind(id).run();
  return (result.meta.changes ?? 0) > 0;
}
...
export async function getAvailabilityInRange(
  ...
  const result = await db
    .prepare(
      `SELECT member_id, date FROM print_member_availability
       WHERE date >= ? AND date <= ?
...
export async function getAvailableMemberIdsOnDate(db: D1Database, date: string): Promise<string[]> {
  const result = await db
    .prepare(`SELECT member_id FROM print_member_availability WHERE date = ?`)
    .bind(date)
...
export async function hasStaffOnDate(db: D1Database, date: string): Promise<boolean> {
  const row = await db
    .prepare(`SELECT 1 AS ok FROM print_member_availability WHERE date = ? LIMIT 1`)
    .bind(date)
    .first<{ ok: number }>();
  return !!row;
}
...
export async function getDatesWithStaffInRange(
  ...
  const result = await db
    .prepare(
      `SELECT DISTINCT date FROM print_member_availability
       WHERE date >= ? AND date <= ?
       ORDER BY date`
    )
    .bind(startDate, endDate)
...
export async function setMemberAvailability(
  ...
  if (available) {
    const stmt = db.prepare(
      `INSERT OR IGNORE INTO print_member_availability (member_id, date) VALUES (?, ?)`
    );
  ...
  await db
    .prepare(
      `DELETE FROM print_member_availability WHERE member_id = ? AND date IN (${placeholders})`
    )
    .bind(memberId, ...dates)
    .run();
}
...
export async function toggleMemberAvailability(
  ...
  if (existing) {
    await db
      .prepare(`DELETE FROM print_member_availability WHERE member_id = ? AND date = ?`)
      .bind(memberId, date)
      .run();
    return false;
  }
  ...
  await db
    .prepare(`INSERT INTO print_member_availability (member_id, date) VALUES (?, ?)`)
    .bind(memberId, date)
    .run();
  return true;
}
...
export async function getAssignedReservationsByDate(
  ...
  const result = await db
    .prepare(
      `SELECT * FROM print_reservations
       WHERE desired_date = ?
         AND status IN ('accepted', 'printing')
         AND print_staff_member_id IS NOT NULL
       ORDER BY created_at`
    )
    .bind(date)
    .all<Reservation>();
...
export async function deleteReservation(db: D1Database, id: string): Promise<boolean> {
  const result = await db.prepare('DELETE FROM print_reservations WHERE id = ?').bind(id).run();
  return (result.meta.changes ?? 0) > 0;
}
...
export async function getUploadSession(db: D1Database, id: string): Promise<UploadSession | null> {
  return db.prepare('SELECT * FROM print_upload_sessions WHERE id = ?').bind(id).first<UploadSession>();
}
...
export async function createUploadSession(db: D1Database, session: UploadSession): Promise<void> {
  await db
    .prepare(
      `INSERT INTO print_upload_sessions (id, upload_id, r2_key, filename, total_size, part_size, parts_json, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
    )
    .bind(
      session.id,
...
export async function updateUploadSession(
  ...
  await db
    .prepare('UPDATE print_upload_sessions SET parts_json = ?, status = ? WHERE id = ?')
