// functions/lib/3dprint/print-video.ts
import type { Env, SessionUser } from '../types';
import { getPrintVideoStoragePath } from './print-app-settings';
...
import { parsePrinterCapabilities } from './printer-capabilities';
...
export async function resolveRequestPrintVideo(
  ...
  if (!printer) {
    return { ok: false, error: '指定されたプリンターが見つかりません' };
  }

  const caps = parsePrinterCapabilities(printer.capabilities_json);
  if (!caps.can_record_print_video) {
    return { ok: false, error: '選択したプリンターは印刷中の動画撮影に対応していません' };
  }
...
export async function uploadPrintVideoToStorage(
  ...
  if (!basePath) {
    throw new Error('印刷動画の保存先が設定されていません。管理画面の設定から指定してください');
  }

  const target = buildPrintVideoRelativeDir(basePath, reservation.id);
  if (!target) {
    throw new Error('印刷動画の保存先パスが不正です');
  }
