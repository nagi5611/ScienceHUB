// functions/lib/simulation/openfoam-job-artifacts.ts
import { sha256HexFromBuffer } from "./openfoam-content-hash";
import { getOpenfoamJobById, updateOpenfoamJobStatus, type OpenfoamJob, type OpenfoamJobArtifactFlags } from "./openfoam-jobs";

export type { OpenfoamJobArtifactFlags };

const MAX_OUTPUT_HASH_BYTES = 200 * 1024 * 1024;

/** Syncs output size from R2 and returns whether artifacts are downloadable. */
export async function syncOpenfoamJobArtifacts(
  env: { FILES: R2Bucket; DB: D1Database },
  job: OpenfoamJob
): Promise<{ job: OpenfoamJob; artifacts: OpenfoamJobArtifactFlags }> {
  let hasOutput = false;
  let hasLog = false;
  ...
  if (job.output_r2_key) {
    ...
    if (head && head.size > 0) {
      ...
      if (sizeChanged || outputSha256) {
        await updateOpenfoamJobStatus(env.DB, job.id, job.status, {
