...
const SCALE_SHORT: Record<SimScale, string> = {
  small: 'S',
  medium: 'M',
  large: 'L',
};
...
export function isGoogleCalendarConfigured(env: GoogleCalendarEnv): boolean {
  return !!(
    env.GOOGLE_SIMULATION_CALENDAR_ID?.trim() &&
    env.GOOGLE_SIMULATION_SERVICE_ACCOUNT_EMAIL?.trim() &&
    env.GOOGLE_SIMULATION_PRIVATE_KEY?.trim()
  );
}
...
export async function testGoogleCalendarConnection(
  ...
  const calendarId = env.GOOGLE_SIMULATION_CALENDAR_ID!.trim();
...
export async function createCalendarEventForReservation(
  ...
  const staffLabel = reservation.sim_staff_member_id
    ? memberMap.get(reservation.sim_staff_member_id)
    : null;
  const staffText = staffLabel ? formatMemberLabel(staffLabel) : '未割り当て';

  const summary = `${SCALE_SHORT[reservation.sim_scale]} ${reservation.title}`;
  const description = [
    `依頼者: ${reservation.homeroom} ${reservation.student_number}番 ${reservation.student_name}`,
    `シミュレーション規模: ${SCALE_LABELS[reservation.sim_scale]}`,
    `実行担当: ${staffText}`,
    reservation.sim_notes ? `注意点:\n${reservation.sim_notes}` : null,
  ...
  try {
    const token = await getAccessToken(env);
    const res = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(env.GOOGLE_SIMULATION_CALENDAR_ID!.trim())}/events`,
      {
        method: 'POST',
        headers: {
...
export async function deleteCalendarEvent(
  ...
  try {
    const token = await getAccessToken(env);
    const res = await fetch(
      `https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(env.GOOGLE_SIMULATION_CALENDAR_ID!.trim())}/events/${encodeURIComponent(eventId)}`,
      {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      }
...
function formatGoogleApiError(status: number, detail: string): string {
  if (status === 404) {
    return 'カレンダーが見つかりません。GOOGLE_SIMULATION_CALENDAR_ID と共有設定を確認してください';
  }
  if (status === 403) {
...
async function getAccessToken(env: GoogleCalendarEnv): Promise<string> {
  const jwt = await createServiceAccountJwt(
    env.GOOGLE_SIMULATION_SERVICE_ACCOUNT_EMAIL!.trim(),
    normalizePrivateKey(env.GOOGLE_SIMULATION_PRIVATE_KEY!)
  );
...
async function importPrivateKey(pem: string): Promise<CryptoKey> {
  ...
  if (!pemBody) {
    throw new Error('秘密鍵の内容が空です。GOOGLE_SIMULATION_PRIVATE_KEY を確認してください');
  }
...
function normalizePrivateKey(key: string): string {
  ...
  if (!normalized.includes('BEGIN PRIVATE KEY')) {
    throw new Error(
      'GOOGLE_SIMULATION_PRIVATE_KEY に -----BEGIN PRIVATE KEY----- が含まれていません。JSON の private_key フィールド全体を貼ってください'
    );
  }
