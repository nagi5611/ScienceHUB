...
import { LEAD_TIME_DAYS, SIM_SCALE_WEIGHT, SLOT_CAPACITY } from './constants';
import {
...
export type SimScale = 'small' | 'medium' | 'large';
...
export function calculateSlotUsage(scales: SimScale[]): number {
  return scales.reduce((sum, scale) => sum + (SIM_SCALE_WEIGHT[scale] ?? 1), 0);
}

/** Returns whether the day has any small print booked. */
export function hasSmallOnDay(existingScales: SimScale[]): boolean {
  return existingScales.includes('small');
}

/** Returns whether the day has medium or large booked. */
export function hasMediumOrLargeOnDay(existingScales: SimScale[]): boolean {
...
export function getAvailableScales(
  ...
  capacity: SimulatorDailyCapacity = DEFAULT_PRINTER_DAILY_CAPACITY
): SimScale[] {
...
export function isDayFull(
  ...
  return isSimulatorDayFull(existingScales, capacity);
}
...
export function getRemainingCapacity(existingScales: SimScale[]): number {
