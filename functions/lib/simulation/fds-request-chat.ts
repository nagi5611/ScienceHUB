// functions/lib/simulation/fds-request-chat.ts

import type { Env } from "../types";
import { createId } from "../types";
import { canUserAccessApp } from "../apps";
import { getFdsRequestById, type FdsRequest } from "./fds-requests";
...
export interface FdsRequestMessageRow {
  id: string;
  request_id: string;
...
export interface FdsRequestMessageApiModel {
  id: string;
  request_id: string;
  sender_user_id: string;
...
export class FdsRequestChatAccessError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "FdsRequestChatAccessError";
  }
}

/** Builds R2 key for a chat attachment. */
export function generateFdsChatAttachmentR2Key(
  requestId: string,
  messageId: string,
  filename: string
): string {
  const safe = filename.replace(/[^\w.\-()]/g, "_").slice(0, 200) || "attachment";
  return `fds-requests/${requestId}/chat/${messageId}/${safe}`;
}

/** Returns whether the user may access chat on this request. */
export async function canAccessFdsRequestChat(
  db: D1Database,
  userId: string,
  request: FdsRequest
...
export async function assertFdsRequestChatAccess(
  db: D1Database,
  userId: string,
  requestId: string
): Promise<{ request: FdsRequest; isStaff: boolean; isOwner: boolean }> {
  const request = await getFdsRequestById(db, requestId);
  if (!request) {
    throw new FdsRequestChatAccessError("依頼が見つかりません");
  }

  const access = await canAccessFdsRequestChat(db, userId, request);
  if (!access.allowed) {
    throw new FdsRequestChatAccessError("依頼が見つかりません");
  }
...
function isAttachmentActive(row: FdsRequestMessageRow, nowMs: number): boolean {
...
async function clearExpiredAttachment(db: D1Database, messageId: string): Promise<void> {
  await db
    .prepare(
      `UPDATE sim_fds_request_messages
...
async function expireAttachmentIfNeeded(
  env: Env,
  db: D1Database,
  row: FdsRequestMessageRow
): Promise<FdsRequestMessageRow> {
...
export function formatFdsRequestMessageForApi(
  row: FdsRequestMessageRow,
  options: {
    viewerUserId: string;
  ...
  let attachment: FdsRequestMessageApiModel["attachment"] = null;
  if (hadAttachment) {
...
export async function listFdsRequestMessages(
  env: Env,
  db: D1Database,
  requestId: string,
  options: {
  ...
  let rows: FdsRequestMessageRow[];
  if (after) {
    const cursor = await getFdsRequestMessageById(db, after);
    if (!cursor || cursor.request_id !== requestId) {
      rows =
        (
          await db
            .prepare(
              `SELECT * FROM sim_fds_request_messages
               WHERE request_id = ?
               ORDER BY created_at ASC, id ASC
               LIMIT ?`
            )
            .bind(requestId, limit)
            .all<FdsRequestMessageRow>()
        ).results ?? [];
    } else {
      rows =
        (
  ...
  const refreshed: FdsRequestMessageRow[] = [];
...
export async function getFdsRequestMessageById(
  db: D1Database,
  messageId: string
): Promise<FdsRequestMessageRow | null> {
  return db
    .prepare(`SELECT * FROM sim_fds_request_messages WHERE id = ?`)
    .bind(messageId)
    .first<FdsRequestMessageRow>();
}

/** Creates a chat message (text and/or attachment). */
export async function createFdsRequestMessage(
  env: Env,
  db: D1Database,
  params: {
    requestId: string;
  ...
  const messageId = createId("fdsmsg");
  const createdAt = new Date().toISOString();
  const expiresAt = attachment
    ? new Date(Date.now() + CHAT_ATTACHMENT_TTL_MS).toISOString()
    : null;
  const r2Key = attachment
    ? generateFdsChatAttachmentR2Key(params.requestId, messageId, attachment.filename)
    : null;
  ...
  await db
    .prepare(
      `INSERT INTO sim_fds_request_messages (
  ...
  const row = await getFdsRequestMessageById(db, messageId);
  if (!row) throw new Error("メッセージの保存に失敗しました");
  return row;
}

/** Posts a system-style chat notice from staff. */
export async function postFdsRequestSystemChatMessage(
  db: D1Database,
  params: {
    requestId: string;
    senderUserId: string;
...
export async function getFdsRequestAttachmentForDownload(
  env: Env,
  db: D1Database,
  params: {
    requestId: string;
  ...
  await assertFdsRequestChatAccess(db, params.userId, params.requestId);

  let row = await getFdsRequestMessageById(db, params.messageId);
  if (!row || row.request_id !== params.requestId) {
    throw new FdsRequestChatAccessError("メッセージが見つかりません");
  }
...
export async function listFdsRequestMessagesForApi(
  env: Env,
  db: D1Database,
  params: {
    requestId: string;
...
): Promise<FdsRequestMessageApiModel[]> {
  const rows = await listFdsRequestMessages(env, db, params.requestId, {
    after: params.after,
    limit: params.limit,
  });
  ...
  const nowMs = Date.now();
  return rows.map((row) => {
    const formatted = formatFdsRequestMessageForApi(row, {
...
export async function purgeExpiredFdsChatAttachments(
  ...
  const rows = await db
    .prepare(
      `SELECT id, attachment_r2_key FROM sim_fds_request_messages
