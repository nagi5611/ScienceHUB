...
export async function adminRescheduleReservation(
  ...
  if (!isAdminDateBookable(desiredDate)) {
    throw new Error('希望実施日は当日以降を選択してください');
  }
  ...
  const slotError = await validateReservationSlot(
    env.DB,
    desiredDate,
    reservation.sim_scale,
    reservation.id,
    reservation.simulator_id ?? undefined,
    { isAdmin: true }
  );
  if (slotError) {
  ...
  const hadCalendarEvent =
    (reservation.status === 'accepted' || reservation.status === 'running') &&
  ...
  if (
    (updated.status === 'accepted' || updated.status === 'running') &&
    updated.sim_staff_member_id
  ) {
